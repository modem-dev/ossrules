...
Middleware is a function that receives `(context, next)` and returns a
`Response`. Call `next()` to continue the chain.
...
Use `createRequestListener` to bridge Node's `http` module to the Fetch API
router:
