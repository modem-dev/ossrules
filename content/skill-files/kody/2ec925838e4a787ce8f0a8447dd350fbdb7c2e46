...
`remix/headers` for cache headers instead of hand-formatting strings:
...
A controller mirrors a route map. Each key in `actions` matches a key in the
route definition. Pass `AppContext` as the second generic to `Controller` so
`get(Database)`, `get(Session)`, `get(Auth)`, etc. are typed against your
middleware stack.
...
The `middleware` array on a controller runs for every action in that subtree,
before action-level middleware:
