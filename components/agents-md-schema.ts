...
export function validateAgentsProject(value: unknown, label: string): string[] {
    ...
    if (value.instructionFile !== undefined && !isInstructionSourcePath(value.instructionFile)) {
        at('"instructionFile" must be a safe repository-relative path to the analyzed instruction source.');
    }
